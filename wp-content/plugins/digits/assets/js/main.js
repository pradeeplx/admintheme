jQuery(function() {
    ...
    if(dig_reg_wp_elem.length) {
        ...
        if(dig_reg_wp_elem.find(".dig_wc_reg_form_end").length===0) {
            ...
            dig_reg_wp_elem.find(".bbp-username").find("#user_login").attr('id', 'user_uname');
        }else{
            ...
            success: function (res) {
                ...
                if (res == "1") {
                    ...
                    if(ihc_loginform==10)
                        ...
                        } else window.location.href = dig_mdet.uri;
                    }
...
//// Ultimate user
    ...
    function update_username_field($this){
        ...
        usernameid.bind("keyup change", function (e) {
            ...
            if (jQuery.isNumeric(jQuery(this).val()) || jQuery(this).attr('only-mob')) {
                ...
                else jQuery(this).css({"padding-left": "0.75em"});
    ...
    register.find("input[type='submit'], .woocommerce-Button, button[name='register']").on('click', function (e) {
        ...
        if(jQuery(this).hasClass("otp_reg_dig_wc")){
            ...
            if(!jQuery.isNumeric(mail) && !jQuery.isNumeric(secmail)){
                showDigMessage(dig_mdet.Thisfeaturesonlyworkswithmobilenumber);
                return false;
            }
    ...
    dig_login_wp_elem.on('submit',function(){
        if(logverify==1){
            jQuery(".dig_wc_mobileLogin").trigger('click');
            return false;
        }
    ...
    jQuery("form#your-profile input[type='submit']").on('click', function(){
        wpuseredit = 1;
        update_time_button = jQuery(this);
    ...
    if(jQuery("#dig_wc_check_page").length && dig_mdet.mob_verify_checkout==1 && dig_mdet.mobile_accept>0) {
        ...
        jQuery("#dig_billing_validate_button").on('click', function () {
            update_time_button = jQuery(this);
            billing_page = 1;
            var form = createAccount.closest('form');
    ...
    if (dig_mdet.login_mobile_accept !== 0) {
        ...
        if (bb_wp_lform.length && dig_mdet.login_otp_accept>0) {
            bb_wp_lform.find("input[type='submit']").parent().append("<div class='dig_otp_block'><div class='digor'> "+ dig_mdet.or +"<br /><br /></div>" +
    ...
    jQuery(".wp_reg").on('submit',function(){
        ...
        update_time_button = jQuery(this).find("input[type='submit']");
    ...
    function validateCustomFields(form){
        ...
        if(form.attr('wait')){
            showDigMessage(form.attr('wait'));
            return false;
        }

        if(form.attr('error')){
            showDigMessage(form.attr('error'));
            return false;
        }

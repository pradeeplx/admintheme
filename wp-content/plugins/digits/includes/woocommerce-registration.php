...
function dig_show_extra_profile_fields( $user, $admin = false ) { ?>
    ...
    <table class="form-table">
        <tr>
            ...
            <td style="position: relative;">
                ...
                <input type="hidden" name="dig_old_phone" id="dig_cur_phone"
	...
	wp_enqueue_style( 'digits-login-style', plugins_url( 'digits/assets/css/login.css' ), array(), null, 'all' );
	...
	function wc_create_new_customer( $email, $username = '', $password = '', $args = array() ) {
		...
		if ( empty( $username ) ) {
			...
			if ( is_numeric( $phone_number ) && $useMobAsUname == 1 ) {
				...
				$isMobUsed = 1;
			} else if ( ! empty( $$email ) && $useMobAsUname == 0 ) {

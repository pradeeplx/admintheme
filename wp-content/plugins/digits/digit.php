 ...
 * Description: Expand your website dimensions by providing signup and login using mobile number. User can register himself/herself with just a mobile number.
 * Version: 6.8.4
...
require_once('includes/phandler.php');
...
function digits_update_plugin_meta($plugin_meta, $plugin_file, $plugin_data, $status){
    ...
    if(!isset($plugin_data['slug'])) {return $plugin_meta;}
...
function getCountry()
{
    ...
    if($countrycode_default!=-1){
        ...
        if(!empty($countrycode)) {return $countrycode;}
    }
...
function getUserCountryCodeFunction()
{
    ...
    if($countrycode_default!=-1){
        ...
        if(!empty($countrycode) && checkwhitelistcode($countrycode)) {return $countrycode;}
    }
...
function createCustomerOnOrderPage($noui = false)
{
    ...
    wp_register_script('digits-cco', plugins_url('/assets/js/dig_cco.js', __FILE__), array('jquery'), digits_version(), true);
    ...
    wp_enqueue_style( 'digits-cco-style', plugins_url( 'assets/css/dig_cco.css', __FILE__ ) ,array(),digits_version());


    wp_enqueue_style('digits-login-style', plugins_url('digits/assets/css/login.min.css',__FILE__), array(), digits_version(), 'all');

    digCountry();
    digits_add_style();
    digits_add_scripts();
    wp_register_script('digits-login-script', plugins_url('/assets/js/login.min.js', __FILE__), dig_deps_scripts(), digits_version(), true);
    ...
    $jsData = array(
        ...
        'left' => 0,
        'verify_mobile' => 0,
        'face' => plugins_url('digits/assets/images/face.png'),
        'cross' => plugins_url('digits/assets/images/cross.png'),
...
?>
    ...
    <?php if (!is_user_logged_in()) {
        ...
        <div id="dig-ucr-container" class="<?php if(is_rtl()){echo 'dig_rtl';}?> dig_lrf_box dig_ma-box dig-box <?php echo $digits_modal_class.' '; echo $dig_main_re; if($page_type==2) {echo ' dig_pgmdl_2';}else echo 'dig_pgmdl_1'; ?>" style="display:none;">
...
function dig_login_contents($modal, $type = 1, $page = false)
{
    ...
    wp_enqueue_style('digits-login-style', plugins_url('/assets/css/login.min.css', __FILE__), array(), digits_version(), 'all');
    wp_enqueue_script('digits-login-script', plugins_url('/assets/js/login.min.js', __FILE__), dig_deps_scripts(), digits_version(), true);
    $app = get_option('digit_api');
    $appid = "";
    if ($app !== false) {
        $appid = $app['appid'];
...
{
    if (get_option('digits_wpnotif_pop_redirect', false) == false) {
        update_option('digits_wpnotif_pop_redirect', true);
        wp_redirect(esc_url_raw(admin_url("/admin.php?page=digits_settings&show_notif_modal=1")));
    }
...
function dig_add_menu_css() {
    wp_enqueue_style('digits-settings', plugins_url('/assets/css/settings.css', __FILE__), array(), digits_version(), 'all');
...
function dig_add_gs_css(){
    ...
    nice_select_scr();
    wp_enqueue_style('digits-gs-style', plugins_url('/assets/css/gs.css', __FILE__), array('google-roboto-regular', 'nice-select'), digits_version(), 'all');

    if(is_rtl()) {
        wp_enqueue_style('digits-gs-rtl-style', plugins_url('/assets/css/gs-rtl.css', __FILE__), array('digits-gs-style'), null, 'all');
...
dig_add_gs_css();
    ...
    <style>.notice,.update-nag, .updated{
        ...
        <?php
if(isset($_GET['show_notif_modal'])){
 ?><style>body{overflow: hidden;}</style>
        ...
        <div class="dig-addon-box dig-modal-center_align dig_ma-box dig-box  dig-modal-con-reno" >
           ...
           <a href="https://codecanyon.net/item/wpnotif-wordpress-sms-whatsapp-notifications/24045791?utm_source=digits-settings&utm_medium=launchpop" target="_blank">
                <img src="<?php echo plugins_url('/assets/images/wpnotif-popup.png', __FILE__); ?>" />
             </a>
            </div>
...
<?php
 ...
 <div class="dig_info_notice dig_notice">In case you are using same purchase code on your testing/production server, then make sure to request addon domain from <a href="<?php echo $request_link;?>">here</a>.<form method="post"><button type="submit">x</button><input type="hidden" name="dig_hid_addon_domain_notice"/></form></div>
<?php
}
 ?>
...
<div class="dig_big_preset_show">
            ...
            <div class="dig_ad_left_side_content">
                ...
                <div class="dig_sts_logo">
                    <a href="https://digits.unitedover.com/" target="_blank">
                        <img src="<?php echo plugins_url('/assets/images/Digits_logo.png', __FILE__); ?>" height="36px"/>
                    </a>
                    <ul class="dig_gs_log_men">
                        <li><a class="dig_ngmc" href="https://help.unitedover.com/?utm_source=digits-wp-settings&utm_medium=kb-button" target="_blank"><?php _e('KNOWLEDGEBASE', 'digits'); ?></a></li>
                        ...
                        <li><a class="dig_ngmc" href="https://digits.unitedover.com/changelog/" target="_blank"><?php _e('CHANGELOG', 'digits'); ?></a></li>
        ...
        <style type="text/css">
            ...
            #wpfooter {
                ...
                wp_register_script('digits-upload-script', plugins_url('/assets/js/upload.js', __FILE__), array('jquery'), digits_version(), true);
...
function getGateWayArray()
{
    ...
    $smsgateways = array(
        ...
        'Textlocal' => array('value' => 17, 'inputs' =>
        ...
        'Adnsms' => array('value' => 19, 'inputs' =>
	        array(__('API Key') => array('text' => true, 'name' => 'api_key'),
	              __('API Secret') => array('text' => true, 'name' => 'api_secret')
	              )),

        'Netgsm' => array('value' => 20, 'inputs' =>
	        array(__('Username') => array('text' => true, 'name' => 'username'),
	              __('Password') => array('text' => true, 'name' => 'password'),
	              __('From') => array('text' => true, 'name' => 'from')
	              )),

        'SMSC.ru' => array('value' => 21, 'inputs' =>
	        array(__('Login') => array('text' => true, 'name' => 'login'),
	              __('Password') => array('text' => true, 'name' => 'password'),
	              __('Sender') => array('text' => true, 'name' => 'sender', 'optional' => 1)
	              )),
        'TargetSMS' => array('value' => 22, 'inputs' =>
	        array(__('Login') => array('text' => true, 'name' => 'login'),
	              __('Password') => array('text' => true, 'name' => 'password'),
	              __('Sender') => array('text' => true, 'name' => 'sender')
	              )),

        'Ghasedak' => array('value' => 23, 'inputs' =>
	        array(__('API Key') => array('text' => true, 'name' => 'api_key')
	              )),
        'Farapayamak' => array('value' => 24, 'inputs' =>
	        array(__('Username') => array('text' => true, 'name' => 'username'),
	              __('Password') => array('text' => true, 'name' => 'password'),
	              __('From') => array('text' => true, 'name' => 'from')
	              )),
...
<div class="digits_gateway_container">
                    ...
                    echo 'style="display:none;"';
                        ...
                        <?php
                            if(isset($input['textarea'])){
                                ?>
                                <textarea type="text" id="<?php echo $inputname; ?>" name="<?php echo $inputname; ?>"
                                   class="regular-text"
                                   autocomplete="off"
                                   rows="9"
 ...
 * */
function digit_select_gateway($gatewayAttributes,$digit_tapp = -1){
    $smsgateways = getGateWayArray();
    ...
    <tr>
                <th scope="row" valign="top" style="vertical-align: top;"><label><?php _e('SMS Gateway', 'digits'); ?> </label></th>
    ...
    <select class="digit_gateway" <?php echo $gatewayAttributes;?> autocomplete="off">
                    ...
                    } ?> data-value="13" value="13" han="firebase" data-test="0">FireBase (<?php _e('Free', 'digits'); ?>)</option>
                        <option <?php if ($digit_tapp == 1) {
                        echo 'selected="selected"';
                    } ?> data-value="1" value="1" han="facebook" data-test="0">Account Kit (<?php _e('Free', 'digits'); ?>)</option>
                        <option <?php if ($digit_tapp == 2) {
                        echo 'selected="selected"';
                    } ?> data-value="2" value="2" han="twilio">Twilio</option>
                        <option <?php if ($digit_tapp == 3) {
                        echo 'selected="selected"';
                    } ?> data-value="3" value="3" han="msg91">Msg91</option>
                        <?php
                        ...
                        foreach ($smsgateways as $name => $details) {
                            ...
                            echo '<option data-value="'.$value.'" value="' . $value . '" ' . $sel . ' han="' . strtolower(str_replace(".", "_", strtolower($name))) . '">' . $name . '</option>';
                        }
                        ?>
                    ...
                    <div><span class="dig_current_gateway" style="<?php if ($digit_tapp == 1 || $digit_tapp == 13 || $digit_tapp == -1) {echo 'display:none;';} ?>"><?php printf(__('You should have paid <span>%s</span> plan to use this.', 'digits'), $gatewayName); ?></span>
                    </div>
    ...
    <div class="dig_api_test">
            ...
            <div class="dig_call_test_api">
                ...
                <div class="dig_test_mob_ho">
                        ...
                        <div class="digcon"><div class="dig_wc_countrycodecontainer dig_wc_logincountrycodecontainer" style="display: inline-block;">
                        <input dig-save="0" type="text" name="digt_countrycode" class="input-text countrycode dig_wc_logincountrycode" value="<?php echo $countrycode;?>" maxlength="6" size="3" placeholder="<?php echo $countrycode;?>">
                        </div>
                        <input dig-save="0" class="mobile" type="text" placeholder="<?php _e('Your Mobile Number','digits');?>" value="<?php echo esc_attr( get_the_author_meta( 'digits_phone_no', get_current_user_id() ) ); ?>" name="mobile/email" style="padding-left:107px !important;"></div>
    ...
    <div class="dig_presets_modal" id="dig_presets_box">
        <div id="dig_presets_modal_box">
            <div id="dig_presets_modal_head">
                ...
                <div id="dig_presets_modal_head_close" class="dig_presets_modal_head_close"><?php _e('CLOSE','digits');?></div>
            </div>
                    ...
                    foreach ($presets_array as $key => $preset_v) {
                        ...
                        <div class="dig_preset_item_list">
                            ...
                            <div class="dig_preset_sel">
                                <img class="dig_preset_sel_tick" src="<?php echo plugins_url('/assets/images/preset-tick.svg', __FILE__); ?>"
                                     draggable="false"/>
                            </div>
                            <div class="dig_preset_img_smp">
                                <img src="<?php echo plugins_url('/assets/images/preset'.$key.'.jpg', __FILE__); ?>"
                                     draggable="false"/>

                                     <a class="dig_preset_big_img" big-href="<?php echo plugins_url('/assets/images/preset'.$key.'.jpg', __FILE__); ?>"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" aria-labelledby="title"
        ...
        <tr>
            ...
            <td class="dig_prst_btns">
                ...
                <input type="hidden"
                       ...
                       id="dig_preset2"/>
                <input type="hidden"
                       value='{"dig_modal_type" : "1", "dig_page_type":"1","bg_image_attachment_id_modal":"","bg_image_attachment_id":"<?php echo plugins_url('/assets/images/bg.jpg', __FILE__); ?>", "backcolor": "#fff","bg_color": "rgba(0,0,0,0)","lbxbg_color": "rgba(17,17,17,0.87)","lb_x": "0","lb_y": "2","lb_blur": "4","lb_spread": "0","lb_radius": "4","lb_color": "rgba(0, 0, 0, 0.5)","fontcolor2": "rgba(51,51,51,1)","fontcolor1": "rgba(255,255,255,1)","bg_color_modal": "rgba(6, 6, 6, 0.8)","lbxbg_color_modal": "#111","lb_x_modal": "0","lb_y_modal": "0","lb_blur_modal": "20","lb_spread_modal": "0","lb_radius_modal": "4","lb_color_modal": "rgba(0, 0, 0, 0.3)","fontcolor1_modal": "rgba(255,255,255,1)","fontcolor2_modal": "rgba(51,51,51,1)"}'
                       id="dig_preset3"/>
                <input type="hidden"
...
function digits_configure_settings()
{
    ...
    <table class="form-table">
        ...
        <tr>
            ...
            <td>
                <select name="dig_mobilein_uname">
                    ...
                    <option value="1" <?php if ($mobInUname == 1) {echo 'selected="selected"';} ?>><?php _e('From Mobile Number', 'digits'); ?></option>
        ...
        <tr>
            <th scope="row"><label><?php _e('Enable /wp-login.php Integeration', 'digits'); ?> </label></th>
            <td>
                <select name="dig_wp_login_inte">
                    <option value="1" <?php if ($wp_login_inte == 1) {echo 'selected="selected"';} ?>><?php _e('Yes', 'digits'); ?></option>
                    <option value="0" <?php if ($wp_login_inte == 0) {echo 'selected="selected"';} ?>><?php _e('No', 'digits'); ?></option>
                </select>
            </td>
        </tr>
    ...
    <table class="form-table">
        ...
        <tr>
            ...
            <td>
                ...
                <select name="whitelistcountrycodes[]" class="whitelistcountrycodeslist dig_multiselect_enable" multiple="multiple">
                    <?php
        ...
        <tr>
            ...
            <td>
                ...
                <select name="blacklistcountrycodes[]" class="blacklistcountrycodes dig_multiselect_enable" multiple="multiple">
                    <?php
        ...
        <div id="digits_hidecountrycode">
        <table class="form-table">
        <tr>
            <th scope="row"><label><?php _e('Hide Country Code', 'digits'); ?> </label></th>
            <td>
                <select name="dig_hide_countrycode" id="dig_hide_countrycode">
                    <option value="1" <?php if ($dig_hide_countrycode == 1) {echo 'selected="selected"';} ?>><?php _e('Yes', 'digits'); ?></option>
                    <option value="0" <?php if ($dig_hide_countrycode == 0) {echo 'selected="selected"';} ?>><?php _e('No', 'digits'); ?></option>
                </select>
             </td>
        </tr>
        </table>
...
<table class="form-table">
    ...
    <table <?php echo $showWC; ?> class="form-table">
        <tr>
            <th scope="row"><label class="no-top"><?php _e('Redirect WooCommerce account page to Digits login page', 'digits'); ?> </label></th>
            <td>
                <select name="dig_redirect_wc_to_dig">
                    <option value="1" <?php if ($dig_redirect_wc_to_dig == 1) {echo 'selected="selected"';} ?>><?php _e('Yes', 'digits'); ?></option>
                    <option value="0" <?php if ($dig_redirect_wc_to_dig == 0) {echo 'selected="selected"';} ?>><?php _e('No', 'digits'); ?></option>
                </select>
            </td>
        </tr>
        <tr>
    ...
    <div class="dig_ad_head"><span><?php _e('Settings Export/Import', 'digits'); ?></span></div>
    ...
    <table class="form-table dig_cs_re">
        <tr>
            ...
            <td>
                ...
                <button id="digits_configuration_export" class="button" type="button"><?php _e('Export','digits');?></button>
                <button id="digits_configuration_import" class="button" type="button"><?php _e('Import','digits');?></button>
            </td>
        </tr>
    </table>
    ...
    <div class="dig_presets_modal dig_overlay_modal_content" id="dig_export_import_content">
        <div id="dig_presets_modal_box">
            <div id="dig_presets_modal_body" class="form-table">
                ...
                <textarea class="dig_export_import_values" placeholder="<?php _e('Paste your import code here...','digits') ?>"></textarea>

                <div class="dig_ex_imp_bottom">
                    <button class="imp_exp_button button imp_exp_btn_fn" type="button" attr-export="<?php _e('Copy','digits');?>"></button>
                    <div class="imp_exp_button imp_exp_cancel dig_presets_modal_head_close" id="dig_presets_modal_head_close"><?php _e('CLOSE','digits');?></div>
                </div>
...
add_action('admin_head', 'digits_add_color_picker');
...
function digits_add_color_picker($hook)
{
    ...
    if (is_admin()) {
        ...
        wp_enqueue_script('rubaxa-sortable', plugins_url('/assets/js/sortable.min.js', __FILE__), null);



        wp_enqueue_script('slick', plugins_url('/assets/js/slick.min.js', __FILE__), null);




        // Include our custom jQuery__FILE__ with WordPress Color Picker dependency
        wp_register_script('digits-script', plugins_url('/assets/js/settings.js', __FILE__), array('jquery','select2','updates', 'wp-color-picker', 'rubaxa-sortable','slick'), digits_version(), true);
...
function digits_add_style()
{
    ...
    wp_register_style('digits-style', plugins_url('/assets/css/main.min.css', __FILE__), array(), digits_version(), 'all');


    wp_enqueue_style('digits-login-style', plugins_url('digits/assets/css/login.min.css'), array(), digits_version(), 'all');
...
function dig_ro_exclude_files( $excluded_files = array() ) {
	$excluded_files[] = 'https://www.gstatic.com/firebasejs/5.5.5/firebase-app.js';
	$excluded_files[] = 'https://www.gstatic.com/firebasejs/5.5.5/firebase-auth.js';
...
function digits_reg_firebase_admin(){
...
add_action( 'admin_enqueue_scripts','digits_reg_firebase_admin' );
...
function iniFireBaseinit()
{
    ...
    if(empty($firebase['api_key']) && empty($firebase['config']) ){return;}
...
function dig_deps_scripts()
{
    ...
    $re = array('jquery', 'scrollTo', 'nice-select', 'password-strength-meter','libphonenumber-mobile');
    if (dig_is_gatewayEnabled(13)) {
        digits_reg_firebase_script();
...
function digits_add_scripts($usercode = 0)
{
    ...
    wp_register_script('scrollTo', plugins_url('/assets/js/scrollTo.js', __FILE__), array('jquery'), digits_version(), true);

    wp_register_script('digits-main-script', plugins_url('/assets/js/main.min.js', __FILE__), dig_deps_scripts(), digits_version(), true);
...
add_action( 'admin_init', 'digits_add_scripts');
...
function digits_login()
{
    ...
    if (isset($_POST['forgotmail']) && $digforgotpass == 1) {
        ...
        if (empty($code) && empty($dig_otp)) {
            $user_login = sanitize_mobile_field_dig($_POST['forgotmail']);
    ...
    if (isset($_POST['digits_reg_mail']) && isset($_POST['dig_nounce']) && $users_can_register == 1) {
        ...
        if ($mobileaccp == 1 && !is_numeric($m) && stripslashes($m) == $mail) {$m = '';}
        ...
        if ((!empty($code) || !empty($otp)) && $mobileaccp > 0) {
            ...
            if(is_numeric(sanitize_mobile_field_dig($m))){
                ...
                $countrycode = sanitize_text_field($_REQUEST['digregscode2']);
            }
    ...
    if (dig_is_gatewayEnabled(1)) {
        wp_register_script('account-kit', 'https://sdk.accountkit.com/' . dig_get_accountkit_locale() . '/sdk.js', array(), digits_version(), false);
    }
    wp_register_script('digits-login-script', plugins_url('/assets/js/login.min.js', __FILE__), dig_deps_scripts(), digits_version(), true);

    wp_register_style('digits-main-login-style', plugins_url('/assets/css/login_body.css', __FILE__), array(), digits_version(), 'all');
    wp_register_style('digits-login-style', plugins_url('/assets/css/login.min.css', __FILE__), array(), digits_version(), 'all');
 ...
 ?>
    ...
    <body>
<div class="dig_bdy_container <?php if($page_type==2) {echo 'dig_ul_divd';}?>">
    <?php if($page_type==2){
    ...
    <div class="dig_lrf_box dig_ma-box <?php if($page_type==2) {echo 'dig_pgmdl_2';}else echo 'dig_pgmdl_1';?>">
    ...
    <div class="dig_clg_bx" style="opacity: 0;">
        <div class="<?php if(is_rtl()){echo 'dig_rtl';}?> dig-container dig_lrf_box dig_ma-box <?php echo $theme; ?> <?php if ($page == 2) {echo 'dig-min-het';} ?>">
             ...
             *
             */
            $redirect_to = apply_filters( 'login_redirect', '', $requested_redirect_to, $user );
            ...
            <div class="digloginpage" <?php if ($page != 1) {echo 'style="display: none;"';} ?>>
                <form method="post">
                    ...
                    <div class="minput">
                        <input type="text" name="mobmail" class="mobile_field mobile_format dig-mobmail"  value="<?php if (isset($_POST['mobmail'])) {
                    ...
                    <input type="hidden" name="digits_redirect_page" value="<?php echo esc_attr( $redirect_to ); ?>" />
                    ...
                    <form method="post" class="digits_register">
                        ...
                        if ($emailaccep > 0 && $mobileaccp > 0) {
                            ...
                            <div class="dig_cs_email minput dig-mailsecond" <?php if ($emailaccep != 2 && $mobileaccp != 2) {
                                ...
                                <label><span class="dig_secHolder"><?php echo $emailmob; ?></span> <?php echo $reqoropt; ?>
                                </label>
                                <span class="<?php echo $bgtype; ?>"></span>
                            </div>
                            <?php
                ...
                <div class="forgot" <?php if ($page == 3) {echo 'style="display:block;"';} ?>>
                    <form method="post">
                        ...
                        <div class="minput forgotpasscontainer" >
                            <input type="text" class="mobile_field mobile_format forgotpass" name="forgotmail" required/>
...
<a href="<?php echo $current_url; ?>" class="dig_page_cancel_color" <?php if (!empty($backcolor)) {echo 'style="color:' . $backcolor . ';"';} ?>><span><?php _e("Cancel", "digits"); ?></span></a>
</div>
<?php } ?>
    </div>
...
function validate_digp_reg_fields($reg_custom_fields, $error, $captcha = true)
{
    ...
    if(empty($reg_custom_fields)){return $error;}
    ...
    foreach ($reg_custom_fields as $label => $values) {
        ...
        if(!array_key_exists($type,$digits_fields)){continue;}
        ...
        if(!$e_value) {$e_value = $_POST[$post_index];}
            ...
            if ($type == "captcha") {
                ...
                if ( $type == "dropdown" || $type == "radio" ) {
                    ...
                    if($required==0 && empty($e_value)){
                        continue;
                    }else if (!in_array($e_value, $options)) {
...
function dig_check_required($values){
    if(function_exists('is_checkout')){
        if(is_checkout()){
             if($values['meta_key']=='last_name' || $values['meta_key']=='first_naem'){
                 return 0;
             }
        }
    }
...
function dig_show_fields($reg_custom_fields,$show_asterisk, $login_page = 1, $bgtype = null, $user_id = 0){
    ...
    if(empty($reg_custom_fields)){return;}
    ...
    foreach ($reg_custom_fields as $label => $values) {
        ...
        if ($type == "textarea") {
            ...
            <textarea type="<?php echo $type; ?>" name="digits_reg_<?php echo $meta_key; ?>"
                      id="digits_reg_<?php echo $meta_key.$rand; ?>" <?php echo $custom_class; ?> <?php if ($values['required'] == 1) {echo "required";} ?>
            ...
            $roles = $wp_roles->roles;
                ...
                <select name="digits_reg_<?php echo $meta_key; ?>" <?php echo $custom_class; ?> <?php if ($values['required'] == 1) {echo "required";} ?>>
                    ...
                    foreach ($values['options'] as $option) {
                        ...
                        echo "<option " . $selected . " value='".$option."'>" . __($roles[$option]['name'],'digits') . "</option>";
                    }
                    ?>
                </select>
        ...
        if ($login_page == 1) {
            if(dig_custom_show_label($type)) {
                ...
                ?>
                <label <?php echo $str; ?> <?php if (!empty($lb_class)) {echo 'class="' . $lb_class . '"';} ?>><?php _e($values['label'], "digits");
                    echo $asterisk; ?></label>
                <?php
            }
            if ($type != "dropdown") {
...
function digits_update_data($gs)
{
    ...
    if (isset($_POST['dig_save'])) {
        ...
        foreach ($smsgateways as $name => $details) {
            ...
            foreach ($details['inputs'] as $inputlabel => $input) {
                ...
                if(isset($input['textarea'])){
                    $inputValue = sanitize_textarea_field($_POST[$name . "_" . $input['name']]);
                }else{
    ...
    if (isset($_POST['enable_createcustomeronorder']) && isset($_POST['defaultuserrole'])) {
        ...
        if (isset($_POST['blacklistcountrycodes'])) {
            ...
            $blacklistcountrycodes = sanitize($_POST['blacklistcountrycodes']);
...
function setup_wizard_header()
{
    ...
    <head>
        ...
        wp_register_style('digits-gs-style', plugins_url('/assets/css/gs.css', __FILE__), array('google-roboto-regular', 'nice-select'), digits_version(), 'all');
    ...
    wp_register_script('digits-script', plugins_url('/assets/js/settings.js', __FILE__), array('jquery', 'updates','wp-color-picker', 'rubaxa-sortable', 'slick','nice-select'), false, true);
...
function digits_presets_custom_fields(){
    return array(
        array('type'=>'text','values'=> array('label'=>__('Last Name','digits'),'required'=> 1,'custom_class'=>'','meta_key' => 'last_name')),
        array('type'=>'user_role','values'=> array('label'=>__('User Role','digits'),'required'=> 1,'custom_class'=>'','meta_key' => 'user_role')),
        array('type'=>'text','values'=> array('label'=>__('Display Name','digits'),'required'=> 1,'custom_class'=>'','meta_key' => 'display_name')),
        array('type'=>'text','values'=> array('label'=>__('Company','digits'),'required'=> 1,'custom_class'=>'','meta_key' => 'billing_company')),
        array('type'=>'text','values'=> array('label'=>__('Address Line 1','digits'),'required'=> 1,'custom_class'=>'','meta_key' => 'billing_address_1')),
        array('type'=>'text','values'=> array('label'=>__('Address Line 2','digits'),'required'=> 1,'custom_class'=>'','meta_key' => 'billing_address_2')),
        array('type'=>'text','values'=> array('label'=>__('City','digits'), 'required'=> 1,'custom_class'=>'','meta_key' => 'billing_city')),
        array('type'=>'text','values'=> array('label'=>__('State','digits'),'required'=> 1,'custom_class'=>'','meta_key' => 'billing_state')),
        array('type'=>'text','values'=> array('label'=>__('Country','digits'),'required'=> 1,'custom_class'=>'','meta_key' => 'billing_country')),
        array('type'=>'text','values'=> array('label'=>__('Postcode / ZIP','digits'),'required'=> 1,'custom_class'=>'','meta_key' => 'billing_postcode')),
...
function digits_customfieldsTypeList()
{
    ...
    return array('text' => array('name' => __('Text','digits'), 'force_required' => 0, 'meta_key' => 1, 'options' => 0, 'slug' => 'text'),
        'textarea' => array('name' => __('TextArea','digits'), 'force_required' => 0, 'meta_key' => 1, 'options' => 0, 'slug' => 'textarea'),
        'number' => array('name' => __('Number','digits'), 'force_required' => 0, 'meta_key' => 1, 'options' => 0, 'slug' => 'number'),
        'dropdown' => array('name' => __('DropDown','digits'), 'force_required' => 0, 'meta_key' => 1, 'options' => 1, 'slug' => 'dropdown'),
        'checkbox' => array('name' => __('CheckBox','digits'), 'force_required' => 0, 'meta_key' => 1, 'options' => 1, 'slug' => 'checkbox'),
        'radio' => array('name' => __('Radio','digits'), 'force_required' => 0, 'meta_key' => 1, 'options' => 1, 'slug' => 'radio'),
        'tac' => array('name' => __('Terms & Conditions','digits'), 'force_required' => 1, 'meta_key' => 1, 'options' => 0,'slug' => 'tac','pref_label'=>'I Agree [t]Terms and Conditions[/t] & [p]Privacy Policy[/t]'),
        'captcha' => array('name' => __('Captcha','digits'), 'force_required' => 1, 'meta_key' => 0, 'options' => 0, 'slug' => 'captcha'),
        'user_role' => array('name' => __('User Role','digits'), 'force_required' => 1, 'meta_key' => 1, 'options' => 0, 'slug' => 'user_role','hidden'=>1,'user_role' => 1),
        'date' => array('name' => __('Date','digits'), 'force_required' => 0, 'meta_key' => 1, 'options' => 0, 'slug' => 'date'),
    );
 ...
 ?>
                ...
                <td>
                    <select name="dig_login_rememberme"
                        ...
                        <option value="2" <?php if ($remember_me == 2) {echo 'selected';} ?>><?php _e('Always', 'digits'); ?></option>
                        <option value="1" <?php if ($remember_me == 1) {echo 'selected';} ?>><?php _e('Yes', 'digits'); ?></option>
                        <option value="0" <?php if ($remember_me == 0) {echo 'selected';} ?>><?php _e('No', 'digits'); ?></option>
                    </select>
                </td>
            </tr>
    ...
    <table class="form-table dig-reg-fields <?php if (is_rtl()) {echo 'dig_rtl';} ?>" id="dig_custom_field_table">
        ...
        if(!empty($reg_custom_fields)) {
            ...
            foreach ($reg_custom_fields as $label => $values) {
                ...
                ?>
                <tr id="dig_cs_<?php echo cust_dig_filter_string($label); ?>" class="dig_field_type_<?php echo strtolower($values['type']);?>" dig-lab="<?php echo $label; ?>">
...
function dig_config_scripts()
{
    ...
    wp_enqueue_script('wp-color-picker-alpha', plugins_url('/assets/js/wp-color-picker-alpha.min.js', __FILE__, array('jquery'), null, true)
...
function digit_addons($active_tab){
	...
	$data = doCurl("https://digits.unitedover.com/?get=products&type=json&data=addons&version=".digits_version()."&purchasecode=".dig_get_option('dig_purchasecode'));
...
if(!empty($purchased_addons)) {
	...
	echo '<div class="dig_addons_title">' . __( 'Active Addons', 'digits' ) . '</div>';
            ...
            <div class="dig-addon-par">
                ...
                <table>
                    <tr>
                        ...
                        <td class="dig_addon_int_btn" data-plugin="<?php echo $plugin['plugin'];?>">
                        ...
                        <input type="hidden" class="dig_plugin_slug"
                                       value="<?php $basename = explode('/', $plugin['plugin']); echo $basename[0];?>">


                                <?php
								if ( is_plugin_active( $plugin['plugin'] ) ) {
                        $addon_settings = call_user_func('digits_addon_'.$key);

									?>
                                    <div class="digmodifyaddon icon-group icon-group-dims" type="-1"></div>
                                    <div class="dig_ngmc updatetabview icon-setting icon-setting-dims <?php echo $active_tab == $addon_settings ? 'dig-nav-tab-active' : ''; ?>" tab="<?php echo $addon_settings;?>tab"></div>

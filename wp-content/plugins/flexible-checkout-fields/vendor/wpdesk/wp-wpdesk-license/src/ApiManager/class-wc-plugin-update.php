...
if ( ! class_exists( 'WPDesk_Update_API_Check' ) ) :
	...
	class WPDesk_Update_API_Check {
			...
			if ( is_object( $response ) ) {
				if ( isset( $response->sections ) ) {
					$response->sections['description']  = apply_filters( 'the_content',
						( isset( $response->sections['description_base64'] ) ? base64_decode($response->sections['description_base64']) : '' ) );
		...
		public function expired_license_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The API key for %s has expired. You can reactivate or purchase a API key from your account <a href="%s" target="_blank">dashboard</a>.',
		...
		public function cancelled_subscription_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s has been cancelled. You can renew the subscription from your account <a href="%s" target="_blank">dashboard</a>. A new API key will be emailed to you after your order has been completed.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->renew_license_url );
		...
		public function no_key_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'A API key for %s could not be found. Maybe you forgot to enter a API key when setting up %s, or the key was deactivated in your account. You can reactivate or purchase a subscription key from your account <a href="%s" target="_blank">dashboard</a>.',
		...
		public function download_revoked_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'Download permission for %s has been revoked possibly due to a API key or subscription expiring. You can reactivate or purchase a API key from your account <a href="%s" target="_blank">dashboard</a>.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->renew_license_url );
		...
		public function no_activation_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( '%s has not been activated. Go to the settings page and enter the API key and subscription email to activate %s.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->product_id );
		...
		public function switched_subscription_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'You changed the subscription for %s, so you will need to enter your new API Key in the settings page. The API Key should have arrived in your email inbox, if not you can get it by logging into your account <a href="%s" target="_blank">dashboard</a>.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->renew_license_url );

...
//			add_action( 'admin_init', array( $this, 'load_settings' ) );
		...
		public function load_settings() {
			...
			add_settings_section( $this->api_manager->api_key,
				__( 'API Key Activation', $this->api_manager->text_domain ), [ $this, 'wc_am_api_key_text' ],
				$this->api_manager->activation_tab_key );
			add_settings_field( 'status', __( 'API Key Status', $this->api_manager->text_domain ),
				[ $this, 'wc_am_api_key_status' ], $this->api_manager->activation_tab_key,
				$this->api_manager->api_key );
			add_settings_field( $this->api_manager->api_key, __( 'API Subscription Key', $this->api_manager->text_domain ),
			...
			add_settings_field( $this->api_manager->activation_email,
				__( 'API Subscription email', $this->api_manager->text_domain ), [ $this, 'wc_am_api_email_field' ],
			...
			add_settings_section( 'deactivate_button',
				__( 'API Key Deactivation', $this->api_manager->text_domain ), [ $this, 'wc_am_deactivate_text' ],
				$this->api_manager->deactivation_tab_key );
			add_settings_field( 'deactivate_button',
				__( 'Deactivate API Key', $this->api_manager->text_domain ),
				...
				if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $api_email == '' || $checkbox_status == 'on' || $current_api_key != $api_key ) {
					...
					if ( $activate_results == false ) {
						add_settings_error( 'api_key_check_text', 'api_key_check_error',
							__( 'Connection failed to the API Key server. Try again later.',
		...
		public function replace_license_key( $current_api_key ) {
			...
			return add_settings_error( 'not_deactivated_text', 'not_deactivated_error',
				__( 'The subscription could not be deactivated. Use the Subscription Deactivation tab to manually deactivate the subscription before activating a new subscription.',
		...
		public function wc_am_deactivate_textarea() {
			...
			echo '/>';
			?><span class="description"><?php _e( 'Deactivates an API Key so it can be used on another blog.',

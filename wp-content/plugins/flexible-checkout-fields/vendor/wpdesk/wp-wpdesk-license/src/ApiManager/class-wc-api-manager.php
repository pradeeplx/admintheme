...
if ( ! class_exists( 'WPDesk_API_Manager_With_Update_Flag' ) ) :
...
//				new WPDesk_API_MENU( $this );
		...
		public function inactive_notice() {
			...
			if (apply_filters('wpdesk_show_plugin_activation_notice', true)) {
				(new AjaxHandler())->hooks();
				new PermanentDismissibleNotice(
					sprintf(
						__( 'The %s%s%s API Key has not been activated, so you won\'t be supported and your plugin won\'t be updated! %sClick here%s to activate the API key and the plugin.',
							$this->text_domain ), '<strong>', $this->software_product_id, '</strong>',
						'<a href="' . esc_url( admin_url( 'admin.php?page=wpdesk-licenses' ) ) . '">', '</a>'
					),
					"notice-{$this->software_product_id}",
					'notice'
				);
            };
		}

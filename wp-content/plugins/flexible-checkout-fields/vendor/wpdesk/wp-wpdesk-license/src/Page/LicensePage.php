...
use WPDesk\License\InstalledPlugins;
...
class LicensePage implements Hookable {
	...
	public function handle_add_page_submenu_item() {
		add_submenu_page( 'wpdesk-helper',
			__( 'Subscriptions', 'wpdesk-helper' ),
			__( 'Subscriptions', 'wpdesk-helper' ),
			'manage_options',
			self::PAGE_SLUG,
			[ $this, 'handle_render_wpdesk_licenses_page' ]
		);
	}
	...
	public function handle_render_wpdesk_licenses_page() {
		...
		if ( ! class_exists( 'WPDesk_Helper_List_Table' ) ) {
	...
	public function handle_css_scripts() {
		...
		if ( isset( $screen ) && in_array( $screen->base,
				[ 'toplevel_page_wpdesk-helper', 'wp-desk_page_wpdesk-licenses', 'wp-desk-1_page_wpdesk-licenses' ],
				true ) ) {
			wp_register_style( 'wpdesk-helper',
				plugins_url( 'wpdesk-helper/assets/css/admin-settings.css' ),
				[],

...
use WPDesk_API_Manager_With_Update_Flag;
...
class InstalledPlugins {
	...
	public function refresh_plugin_update_info() {
		$this->get_plugins_activation_info( true );
	}
	...
	public function get_plugins_activation_info( $hook_to_updates = false ) {
		...
		foreach ( $wpdesk_helper_plugins as $key => $wpdesk_helper_plugin ) {
			...
			$plugins[ $key ][ self::KEY_API_MANAGER ]       = new WPDesk_API_Manager_With_Update_Flag(
				$upgrade_url = '',
			...
			$plugins[ $key ][ self::KEY_ACTIVATION_STATUS ] = get_option(
				$plugins[ $key ][ self::KEY_API_MANAGER ]->activated_key,
				'Deactivated'
			);
		}

 ...
 */
class LibraryDebugPage implements Hookable {
		...
		add_action( 'admin_menu',
			function () {
				...
				add_submenu_page( $parent_slug,
					__( 'Library report', 'wpdesk-helper' ),
					__( 'Library report', 'wpdesk-helper' ),
					'manage_options',
					'wpdesk-helper-library-report',
					function () {
	...
	private function handle_render_library_report_page() {
		...
		print_r( [
			'used_libraries' => $this->library_debug->get_libraries_report( $vendor_files_report ),
			'used_files'     => $vendor_files_report,
		] );
		echo '</pre>';
	}

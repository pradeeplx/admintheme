...
class LibraryDebug {
	...
	private function get_used_wpdesk_vendor_files() {
		...
		$vendor_files = array_filter( $all_files,
			function ( $filename ) {
	...
	public function get_wpdesk_vendor_files_report() {
		...
		foreach ( $vendor_files as $vendor_file ) {
			...
			$loaded[] = [
				'plugin_name'                    => preg_match( '/\/plugins\/([^\/]+)/',
					$vendor_file,
					$matches ) ? $matches[1] : '',
				'plugin_path'                    => $plugin_path,
				self::REPORT_KEY_LIBRARY_NAME    => $library_name,
				self::REPORT_KEY_LIBRARY_VERSION => $this->get_library_version( $plugin_path, $library_name ),
				'file'                           => $vendor_file,
			];
		}
	...
	private function get_library_version( $plugin_path, $library_name ) {
		...
		if ( file_exists( $lock_file ) ) {
			...
			if ( ! isset( $cache[ $lock_file ] ) ) {
				$cache[ $lock_file ] = json_decode( file_get_contents( $lock_file ), true );
			}
			$library_version = array_reduce( $cache[ $lock_file ][ self::COMPOSER_KEY_PACKAGES ],
				function ( $carry, $package_info ) use ( $library_name ) {

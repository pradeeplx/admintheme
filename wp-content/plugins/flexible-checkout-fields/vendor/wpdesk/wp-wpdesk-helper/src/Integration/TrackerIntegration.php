 ...
 */
class TrackerIntegration implements Hookable {
	const PRIORITY_AFTER_LOGS = 35;
	...
	public function hooks() {
		$tracker_enabled = apply_filters( 'wpdesk_tracker_enabled', true );
		if ( $tracker_enabled ) {
			add_action( 'admin_init',
				function () {
					$this->handle_page_settings_track_section();
				} );
			$this->initialize_main_tracker();
		}
	}

	private function initialize_main_tracker() {
		$tracker_factory = new \WPDesk_Tracker_Factory();
		$this->tracker   = $tracker_factory->create_tracker( basename( dirname( __FILE__ ) ) );
	}
	...
	private function handle_page_settings_track_section() {
		add_settings_section( 'wpdesk_helper_tracking',
			__( 'Plugin usage tracking', 'wpdesk-helper' ),
			null,

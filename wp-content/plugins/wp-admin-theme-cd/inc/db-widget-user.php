...
if( ! function_exists('wp_admin_list_online_users') ) :
    ...
    function wp_admin_list_online_users() {
        ...
        foreach( $users as $current_user ) {   
            ...
            if( $is_logged_in != '1' ) {
				if( is_rtl() ) {
					$login_status = '<span class="user-status" style="background:#a5b1c2">' . esc_html__( 'logged out', 'wp-admin-theme-cd' ) . '</span>' . esc_html__( 'is', 'wp-admin-theme-cd' );
				} else {
				...
				if( is_rtl() ) {
					$login_status = '<span class="user-status" style="background:#20bf6b">' . esc_html__( 'logged in', 'wp-admin-theme-cd' ) . '</span>' . esc_html__( 'is', 'wp-admin-theme-cd' );
				} else {
			...
			if( is_rtl() ) {
				echo '<tr><td class="listing-img">' . get_avatar( $current_user->user_email, 64 ) . '</td><td>' . $login_status . '<strong> ' . esc_html( $current_user->display_name ) . '</strong><br><small>' . esc_html( $last_user_login ) . ': ' . esc_html__( 'Last Login', 'wp-admin-theme-cd' ) . '<br>' . esc_html( $last_user_logout ) . ': ' . esc_html__( 'Last Logout', 'wp-admin-theme-cd' ) . '</small></td>';
			} else {

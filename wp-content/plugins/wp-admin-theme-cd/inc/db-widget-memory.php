    ...
    function wp_admin_memory_widget_content() { 
        ...
        <div class="wpat-memory">
            ...
            <div class="wpat-memory-detail">
                ...
                echo esc_html__( 'WP PHP Limit', 'wp-admin-theme-cd' ) . ': ';            
                if( $wp_mem_limit != -1 ) {
                    echo esc_html( $wp_mem_limit ) . ' MB <br>'; 
                } else {
                    echo esc_html__( 'Unlimited', 'wp-admin-theme-cd' ) . ' (' . esc_html( $wp_mem_limit ) . ')<br>'; 
                } ?>
                <div class="wpat-mem">
                ...
                echo esc_html__( 'WP PHP Limit (WP Admin only)', 'wp-admin-theme-cd' ) . ': ';            
                if( $wp_max_mem_limit != -1 ) {
                    echo esc_html( $wp_max_mem_limit ) . ' MB <br>'; 
                } else {
                    echo esc_html__( 'Unlimited', 'wp-admin-theme-cd' ) . ' (' . esc_html( $wp_max_mem_limit ) . ')<br>'; 
                } ?>
                <div class="wpat-mem">
                ...
                echo esc_html__( 'Server PHP Limit', 'wp-admin-theme-cd' ) . ': ';            
                if( $server_mem_limit != -1 ) {
                    echo esc_html( $server_mem_limit ) . ' MB <br>'; 
                } else {
                    echo esc_html__( 'Unlimited', 'wp-admin-theme-cd' ) . ' (' . esc_html( $server_mem_limit ) . ')<br>'; 
                } ?>
                <div class="wpat-mem">
                ...
                if( $mem_warning ) { ?>
                    <div class="wpat-memory-warning">
                        <span class="dashicons dashicons-warning"></span> <?php echo esc_html__( 'Your Server PHP Memory Limit is unlimited. A defined memory limit helps prevent poorly written scripts for eating up all available memory on a server.', 'wp-admin-theme-cd' ); ?>
                    </div>
                <?php } ?>
                ...
                <div class="wpat-memory-infobox">
                    ...
                    if ( false === wp_is_ini_value_changeable( 'memory_limit' ) ) {
                        echo '<span class="dashicons dashicons-no"></span> ' . esc_html__( 'WordPress <strong>can not</strong> change the PHP ini value at runtime.', 'wp-admin-theme-cd' );
                    } else {
                        echo '<span class="dashicons dashicons-yes"></span> ' . esc_html__( 'WordPress can change the PHP ini value at runtime.', 'wp-admin-theme-cd' );
                    } ?> 
                </div>
...
if( ! function_exists('wp_admin_memory_widget') ) :
	...
	function wp_admin_memory_widget() {
		...
		wp_add_dashboard_widget('wp_memory_db_widget', esc_html__( 'WP Memory Usage', 'wp-admin-theme-cd' ), 'wp_admin_memory_widget_content');

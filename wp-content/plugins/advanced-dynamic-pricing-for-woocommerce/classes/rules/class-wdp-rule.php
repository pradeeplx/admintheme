...
class WDP_Rule {
	...
	protected function get_roles_rows( $rule_data ) {
		...
		foreach ( $rows as &$row ) {
			if ( ! isset( $row['roles'], $row['discount_type'], $row['discount_value'] ) && ! is_array( $row['roles'] ) ) {
				unset( $row );
			}
		}
	...
	protected function get_deal_attempt_limit( $set_collection, $cart ) {
		...
		if ( $this->rule_data['get_products']['repeat'] === 'based_on_subtotal' ) {
			if ( isset( $this->rule_data['get_products']['repeat_subtotal'] ) ) {
				...
				if ( $cart->get_wc_cart() ) {
					$subtotal = $cart->get_wc_cart()->get_subtotal();
				}
	...
	protected function get_product( $product_id ) {
		if ( isset( $this->loaded_products[ $product_id ] ) ) {
			return $this->loaded_products[ $product_id ];
		}

		return $this->loaded_products[ $product_id ] = wc_get_product( $product_id );
	}
	...
	protected function match_product_with_filters( $the_product, $cart ) {
		...
		$product_excluding = new WDP_Product_Filtering_new();
		...
		foreach ( $this->get_filters() as $filter_key => $filter ) {
			...
			$matched = $matched && ! $product_exclude;

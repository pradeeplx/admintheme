 ...
 * @var string $table_type product or category
 */
$use_message_as_header = apply_filters( "wdp_use_message_as_{$table_type}_bulk_table_header", false );

if ( $use_message_as_header ) {
    $deal_title = $data['table_message'];
} else {
	if ( 'bulk' === $data['type'] ) {
	...
	<?php if ( $show_footer_in_bulk_table ): ?>
        <span class="wdp_pricing_table_footer">
            <?php if ( ! empty( $data['table_message'] ) ): ?>
	            <?php if ( ! $use_message_as_header ): ?>
                    <p class=""><?php _e( apply_filters( 'wdp_format_bulk_table_message', $data['table_message'] ) ); ?></p>
                <?php endif; ?>
            <?php elseif ( $show_footer_in_bulk_table && ! empty( $dependencies ) ): ?>
                <div>
